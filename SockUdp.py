...
class sockUDP(object):
    ...
    # decode is used to demonstrate the UDP packet
    def decode(self):
        self.pack()
        s = ""
        s = s + "User Data Protocol, Src Port : " + str(self.UDP_SrcPort) + " , Dst Port : " + str(self.UDP_DstPort) + "\n"
        s = s + "\t Source Port : " + str(self.UDP_SrcPort) + "(" +  inttohex(self.UDP_SrcPort,4) + ")\n"
        s = s + "\t Destination Port : " + str(self.UDP_DstPort) + "(" + inttohex(self.UDP_DstPort,4) + ")\n"
        s = s + "\t Length : " + str(self.UDP_Length) + "(" + inttohex(self.UDP_Length,4) + ")\n"
        s = s + "\t Checksum : " + inttohex(self.UDP_CheckSum,4)[2:4] + inttohex(self.UDP_CheckSum,4)[0:2] + "\n"
    ...
    # socksendUDP is used to send to UDP packet
    def sendsockUDP(self):
        UDPsender = IP(dst=self.IP_dst, src=self.IP_src, proto=17) / self.pack()
        #print hexdump(UDPsender)
        send(UDPsender)

    # appendPCAP is used to write the packet into a file
    def appendPCAP(self, filename):
        tmp = Ether(type=0x0800) / IP(dst=self.IP_dst, src=self.IP_src, proto=17) / self.pack()
        SockPcap.PCAPwrite(filename, tmp)

'''
s2 = sockUDP('192.168.1.102', 14005, '6.7.8.9', 8080, '746869736973666F727564707061636B657474657374')
print s2.decode()
print hexdump(s2.pack())
s2.sendsockUDP()
'''
